class District < ActiveRecord::Base
  ...
  def set_coord
    if self.lat.blank? && self.lon.blank?
      url = "http://geocode-maps.yandex.ru/1.x/?format=json&geocode=#{self.name.split.join('+')}&ll=30.327541,59.936952&spn=2,2&rspn=1"
      response = JSON.parse(Net::HTTP.get_response(URI.parse(URI.encode(url))).body)
      self.lon, self.lat = if response['response']['GeoObjectCollection']['featureMember'].present?
